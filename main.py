            ...
            if args.local_rank == 0:
                ...
                if wandb.run:
                    wandb.log({'epoch': epoch + 1,
                               'loss/box': avg_box_loss.avg,
                               'loss/cls': avg_cls_loss.avg,
                               'loss/dfl': avg_dfl_loss.avg,
                               'metrics/mAP': last[0],
                               'metrics/mAP@50': last[1],
                               'metrics/recall': last[2],
                               'metrics/precision': last[3]})
...
def main():
    ...
    parser.add_argument('--weights', default=os.getenv('WEIGHTS', ''), type=str,
    ...
    if args.local_rank == 0 and args.train:
        import wandb
        run_name = f"yolo-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        wandb.init(project=os.getenv('WANDB_PROJECT', 'HierLight-YOLOv11'),
                   name=run_name,
